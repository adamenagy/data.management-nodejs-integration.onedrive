...
router.post('/integration/sendToOnedrive', jsonParser, function (req, res) {
  ...
  var onedriveFolderId = req.body.onedrivefolder;
      ...
      request({
        ...
        var msGraphClient = msGraph.init({
          ...
          authProvider: function (done) {
            done(null, tokenSession.getOnedriveToken())
          }
        })
...
router.post('/integration/sendToAutodesk', jsonParser, function (req, res) {
  ...
  var onedriveFileId = req.body.onedrivefile;
  ...
  switch (autodeskType) {
    case "folders":
      ...
      sendToAutodesk(projectId, folderId, onedriveFileId, fileName, res, req);
      break;
    case "projects":
...
function sendToAutodesk(projectId, folderId, onedriveFileId, fileName, res, req) {
  var tokenSession = new token(req.session);
  ...
  var onedriveDriveId = onedriveFileId.split('!')[0]

  msGraphClient
    .api('/drives/' + onedriveDriveId + '/items/' + onedriveFileId )// + '/content')
    .get(function (error, data) {
      ...
      request({
        ...
        try {
          uploadFile(projectId, folderId, fileName, file.length, file, req)
